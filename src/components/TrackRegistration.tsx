...
import { searchRegistration } from '../api/lookupApi';
import { downloadReceipt } from '../utils/receipt';
import { displayName, categoryLabel, statusLabel } from '../utils/registrationDisplay';
...
export default function TrackRegistration() {
  ...
  async function handleSearch() {
    ...
    if (!q) return;
    setSearching(true);
    const match = await searchRegistration(q);
  ...
  return (
    <div className="card" id="track">
      ...
      <p className="hint">Look up an individual or team entry by reference number or email.</p>

      <div className="search-row">
        <input
          value={query}
          onChange={(e) => setQuery(e.target.value)}
          placeholder="e.g. KICR-10231 or your email"
      ...
      {result && result !== 'not-found' && (
        <div className="summary-row-block">
          ...
          <div className="summary-row"><span>Name</span><strong>{displayName(result)}</strong></div>
          <div className="summary-row"><span>Category</span><strong>{categoryLabel(result)}</strong></div>
